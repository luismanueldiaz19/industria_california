...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Cuentas por Cobrar (CXC) - Neo Project</title>
    <style>
        body { font-family: 'Helvetica', Arial, sans-serif; font-size: 11px; color: #333; margin: 0; padding: 0; }
        .header { width: 100%; border-bottom: 2px solid #28a745; padding-bottom: 15px; margin-bottom: 25px; }
        table { width: 100%; border-collapse: collapse; }
        .company-info { text-align: left; vertical-align: top; }
        .company-name { font-size: 22px; font-weight: bold; color: #28a745; margin-bottom: 5px; }
        ...
        .data-table th { background-color: #28a745; color: white; padding: 10px 8px; text-align: left; font-size: 11px; text-transform: uppercase; }
        ...
        .totals-table .grand-total td { background-color: #28a745; color: white; font-weight: bold; font-size: 14px; border: none; }
        
        .filters { margin-bottom: 20px; font-size: 10px; color: #666; background-color: #f4f6f9; padding: 10px; border-radius: 4px; border-left: 4px solid #28a745; }
...
<body>
        ...
        <table>
            <tr>
                ...
                <td class="report-info" style="width: 40%;">
                    <div class="report-title">Estado de Cuentas por Cobrar</div>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            @forelse($cxcs as $cxc)
            @php 
                $saldo = $cxc->monto_factura - $cxc->monto_pagado;
            ...
            <tr>
                ...
                @endif
                <td>{{ \Carbon\Carbon::parse($cxc->fecha_vencimiento)->format('d/m/Y') }}</td>
                <td class="text-right">${{ number_format($cxc->monto_factura, 2) }}</td>
                <td class="text-right">${{ number_format($cxc->monto_pagado, 2) }}</td>
                <td class="text-right" style="font-weight: bold; color: {{ $saldo > 0 ? '#dc3545' : '#28a745' }};">
                    ${{ number_format($saldo, 2) }}
                </td>
                <td class="text-center">
                    <span class="badge badge-{{ strtolower($cxc->estado) }}">{{ $cxc->estado }}</span>
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="{{ isset($cliente) ? 6 : 7 }}" class="text-center" style="padding: 20px;">No hay cuentas por cobrar registradas.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
        ...
        <div class="totals-container">
            <table class="totals-table">
                ...
                <tr class="grand-total">
                    <td class="label" style="color: white;">SALDO PENDIENTE:</td>
